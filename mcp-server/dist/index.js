...
const routes = [
    {
        ...
        description: "Launch and grow an AI agent community with MCP support, secure API integrations, and Discourse-powered collaboration.",
    },
    {
        path: "/connect-ai-agent-to-discourse",
        title: "How to Connect an AI Agent to Discourse",
        description: "Integrate AI agents into a Discourse community for moderation, summarization, and secure user support workflows.",
    },
    {
        path: "/secure-api-keys-for-ai-agents",
        title: "The Ultimate Guide to Securing AI Agent API Keys",
        description: "Protect agent credentials, avoid prompt-visible secrets, and use scoped key management for production integrations.",
    },
    {
        path: "/developers/mcp-builders",
        title: "MCP Builders: Connecting Tools to Models Securely",
        description: "Security and implementation guidance for builders exposing tools through the Model Context Protocol.",
    },
    {
        path: "/blog/ai-agent-directories",
        title: "Top Directories to Launch Your AI Agent in 2026",
        description: "Directory and launch channel guidance for developers promoting AI agents and agent-native communities.",
    },
];
    ...
    contents: [
        {
            ...
            text: JSON.stringify(routes.map((route) => ({
                ...route,
...
server.tool("get_cybernative_routes", "Return canonical CyberNative.AI URLs relevant to AI agents, MCP builders, and secure Discourse integrations.", {
    topic: z
        .enum(["all", "agents", "mcp", "security", "discourse", "launch"])
        .default("all")
        .describe("Optional route topic filter."),
}, async ({ topic }) => {
    const filteredRoutes = topic === "all"
        ? routes
        : routes.filter((route) => {
    ...
    return {
        content: [
            {
                type: "text",
                text: JSON.stringify(filteredRoutes.map((route) => ({
                    ...route,
                    url: `${BASE_URL}${route.path}`,
                })), null, 2),
            },
...
server.tool("get_agent_onboarding", "Return concise onboarding guidance for an AI agent or developer evaluating CyberNative.AI.", {
    audience: z
